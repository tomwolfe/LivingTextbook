 ...
 * @returns {Blob}
 */
function arrayBufferToBlob(buffer, type = 'image/png') {
 ...
 * @returns {Promise<string>} Cache key (prompt hash)
 */
export async function cacheImage(prompt, imageBlob, metadata = {}) {
  try {
 ...
 */
export async function isImageCached(prompt) {
  try {
    const db = await openDB();
    const promptHash = hashString(prompt);
 ...
 * @returns {Promise<boolean>}
 */
export async function deleteCachedImage(prompt) {
  try {
    const db = await openDB();
    const promptHash = hashString(prompt);
...
export async function getCacheStats() {
  try {
    ...
    return new Promise((resolve, reject) => {
      ...
      countRequest.onsuccess = () => {
        resolve({
          ...
          estimatedSize: 'Unknown', // Would need to iterate all entries
        });
      };
    ...
    return { imageCount: 0, estimatedSize: 'Unknown' };
  }
}
...
/**
 * Get all saved books
 * @returns {Promise<Array>} List of saved books
 */
